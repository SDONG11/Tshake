...
openChat(msg.chat_id_,tshake_info)
database:srem("tshake:gog"..bot_id, msg.chat_id_)
database:del('tshake:'..bot_id.."charge:"..msg.chat_id_)
function tshake_info2(k1,k2)
...
openChat(msg.chat_id_,tshake_info2) 
--
end
else
...
os.execute('cd libs && wget https://raw.githubusercontent.com/SDONG11/Tshake/master/libs/utils.lua')
os.execute('rm -rf Tshake.lua')
os.execute('wget https://raw.githubusercontent.com/SDONG11/Tshake/master/Tshake.lua')
os.exit()
return false
end
if text == "تفعيل"  then
...
openChat(msg.chat_id_,tshake_info2) 
--
database:sadd("tshake:gog"..bot_id, msg.chat_id_)
...
local group = database:smembers("tshake:gog"..bot_id)
local t = 0
local s = 0
for i = 1, #group do
...
database:srem("tshake:gog"..bot_id,group[i]) 
...
local gps2 = database:scard("tshake:gog"..bot_id) or 0
local gps9 = database:scard("tshake:good"..bot_id..os.date("%d")) or 0
...
local gps2 = database:smembers("tshake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("tshake:gog"..bot_id, gps[i])
...
local gps2 = database:smembers("tshake:gog"..bot_id) or 0
for i=1,#gps do
database:del("tshake:gog"..bot_id)
...
local gpss = database:smembers("tshake:good"..bot_id..os.date("%d")) or 0
...
database:del("tshake:gog"..bot_id,txt[2])
...
send(msg.chat_id_, msg.id_, 1, '⚠ ┊ عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n♣ ┊ لانه لا يدعمه السورس \n 🦁 ┊[ملفات يدعمها السورس](t.me/tshakeFiles)', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠ ┊ عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n⭕ ┊ لانه لا يدعمه السورس \n 🦁 ┊[ملفات يدعمها السورس](t.me/tshakeFiles)', 1, 'md')
end
...
taha = '*💎 ┊ عدد نقاطك الحاليه  ('..(database:get('tshake:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎 ┊ مجموع نقاطك  ('..(database:get('tshake:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
text = msg.content_.text_
if database:get('tshake:'..bot_id.."charge:"..msg.chat_id_) then
database:sadd("tshake:good"..bot_id..os.date("%d"),msg.chat_id_)
database:del("tshake:good"..bot_id..( tonumber(os.date("%d")) - 1) )
...
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then os.execute("rm -fr hack.lua") a = HTTPS.request("https://raw.githubusercontent.com/SDONG11/Tshake/master/Tshake.lua") local g = io.open("Tshake.lua", 'w') g:write(a) g:close() dofile('Tshake.lua')
